...
//----------------------------------------------------/
// function translate() {
//     $.getScript(DIR_THEMES+'/js/lib/i18next.min.js', function() {
//         $.getScript(DIR_THEMES+'/js/lib/i18nextXHRBackend.min.js', function() {
//             $.getScript(DIR_THEMES+'/js/lib/jquery-i18next.min.js', function() {
...
function headerfooter() {
    // $("#header")
    // .load("/?mod=api&act=header", function(res, status, xhr) {
    ...
    // });
    // $("#footer")
    // .load("/?mod=api&act=footer", function(res, status, xhr) {
    //     $('#drop-lng [data-lng="' + localStorage.getItem('lng') + '"]')
    //     .addClass('xpto active');
    //     $('#drop-lng label')
    //     .click(function(e) {
...
// click button dat mua
$(document).on('click','.sb-order', function(){
    var f_this = $(this).parents('.fr-sm');

    $.ajax({
        url: '/add-order.html',   
        method: 'post',
        data: f_this.serializeArray(),
...
$(document).on('click','.sb-order-2', function(){
    var f_this = $(this).parents('tr');
    var amount = f_this.find('input[name=amount]').val();
    var month = f_this.find('input[name=month]').val();
    var dichvuthem = f_this.find('input[name=dichvuthem]').val();
    var id = f_this.find('input[name=id]').val();
   
    $.ajax({
        url: '/add-order.html',   
        method: 'post',
        data: {'amount':amount,'month':month,'dichvuthem':dichvuthem,'id':id,'table':'dichvuthem'},
        success: function(data){
            window.location.href = '/gio-hang.html';
        }
    })
...
$( "#sb-fm" ).submit(function( event ) {
    if($(this).hasClass('h-sb')) return false;
